...
echo -n "Enter Site Domain Name (do not include http://) [${siteurl}]: "
read inputsiteurl
...
installFile=$(mktemp -t XXXXX.php);
curl -sS -o $installFile https://raw.githubusercontent.com/PufferPanel/Tools/master/installer/versions/${ppversion}.php

php -f $installFile mysqlHost="$mysqlhost" mysqlUser="$mysqluser" mysqlPass="$mysqlpass" companyName="$companyname" siteUrl="$siteurl" adminName="$adminname" adminEmail="$adminemail" adminPass="$adminpass" installDir="$installpath"
if [ $? -ne 0 ]; then
    echo -e "${red}An error occured while installing, halting${normal}";
    revertInstall
    exit
...
if $skiplang; then
    ...
    curl -sS https://raw.githubusercontent.com/PufferPanel/Tools/master/language/language-builder | bash -s -- -p $installpath
fi
